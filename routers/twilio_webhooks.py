...
@router.post("/voice-simple-test")
async def voice_simple_test(request: Request):
    """Dead simple test - just say hello, no Sarvam AI, no DB lookup"""
    form_data = await request.form()
    print(f"\n🔔 SIMPLE TEST WEBHOOK CALLED")
    print(f"Form data: {dict(form_data)}")
    return Response(
        content="<Response><Say voice='Polly.Aditi-Neural' language='en-IN'>Hello! The webhook is working. Sarvam AI backend is connected successfully.</Say><Hangup/></Response>",
...
async def handle_generic_voice_webhook(request: Request):
    """
    Generic voice webhook - called by Twilio when call is answered
    """
    print(f"\n{'='*60}")
    ...
    try:
        ...
        print(f"CallSid: {twilio_call_sid}, Status: {call_status}")
        print(f"All params: {dict(form_data)}")

        # Look up our call
        call_data = real_call_service.get_call_by_twilio_sid(twilio_call_sid)

        webhook_base = os.getenv("WEBHOOK_BASE_URL", "https://sarvam-ai-backend-nj0q.onrender.com")

        if not call_data:
            print(f"⚠️ Call not found for SID: {twilio_call_sid}")
            # Still greet with generic message
            twiml = f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Gather input="speech" action="{webhook_base}/api/twilio/process-speech" method="POST" timeout="5" speechTimeout="auto" language="en-IN">
    <Say voice="Polly.Aditi-Neural" language="en-IN">Hello! This is Sarvam AI calling. Am I speaking with the right person?</Say>
  </Gather>
  <Say voice="Polly.Aditi-Neural" language="en-IN">I did not hear you. Goodbye!</Say>
</Response>"""
            return Response(content=twiml, media_type="application/xml")

        call_id = call_data["call_id"]
        lead_name = call_data["lead_info"].lead_name
        print(f"✅ Found call_id: {call_id}, lead: {lead_name}")

        # Use Twilio TTS for greeting (fast, no external API needed)
        # Sarvam AI will be used for conversation responses
        webhook_base = os.getenv("WEBHOOK_BASE_URL", "https://sarvam-ai-backend-nj0q.onrender.com")
        twiml = f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Gather input="speech" action="{webhook_base}/api/twilio/process-speech" method="POST" timeout="5" speechTimeout="auto" language="en-IN" hints="yes,no,interested,not interested,tell me more,price,demo">
    <Say voice="Polly.Aditi-Neural" language="en-IN">Hello {lead_name}! This is Sarvam AI calling. Am I speaking with the right person?</Say>
  </Gather>
  <Redirect method="POST">{webhook_base}/api/twilio/voice</Redirect>
</Response>"""

        print(f"✅ Returning TwiML greeting for {lead_name}")
        return Response(content=twiml, media_type="application/xml")
        ...
        return Response(
            content='<?xml version="1.0" encoding="UTF-8"?><Response><Say>Hello! This is Sarvam AI. Please hold.</Say></Response>',
            media_type="application/xml"
        )
...
async def process_speech_generic(request: Request):
    """
    Generic speech processing - called after lead speaks
    """
    try:
        form_data = await request.form()
        CallSid = form_data.get("CallSid", "")
        SpeechResult = form_data.get("SpeechResult", "")
        print(f"🗣️ Speech received - SID: {CallSid}, Text: '{SpeechResult}'")
        ...
        if not call_data:
            print(f"⚠️ Call not found for SID: {CallSid}")
            return Response(
                content=f'<?xml version="1.0" encoding="UTF-8"?><Response><Say voice="Polly.Aditi-Neural" language="en-IN">Thank you for your time. Goodbye!</Say><Hangup/></Response>',
                media_type="application/xml"
            )

        call_id = call_data["call_id"]
        lead_speech = SpeechResult or "I didn't catch that"
        print(f"✅ Processing speech for call_id: {call_id}: '{lead_speech}'")

        # Check end-call keywords
        end_keywords = ['not interested', 'no thanks', 'goodbye', 'bye', 'hang up', 'stop', 'busy']
        should_end = any(kw in lead_speech.lower() for kw in end_keywords)
        ...
        current_turns = len([m for m in call_data["conversation_history"] if m["role"] == "lead"])
        if current_turns >= max_turns or should_end:
            return Response(
                content=f'<?xml version="1.0" encoding="UTF-8"?><Response><Say voice="Polly.Aditi-Neural" language="en-IN">Thank you so much for your time. Have a wonderful day!</Say><Hangup/></Response>',
                media_type="application/xml"
            )

        # Add lead message to history
        call_data["conversation_history"].append({"role": "lead", "content": lead_speech})
        ...
        try:
            ...
            ai_text = await sarvam_ai_service.generate_response(
                lead_message=lead_speech,
                conversation_history=call_data["conversation_history"],
                lead_info=call_data["lead_info"].dict(),
                product_info=product_info
            )
            print(f"✅ Sarvam AI LLM response: {ai_text}")
        except Exception as llm_err:
            print(f"⚠️ Sarvam AI LLM failed: {llm_err}, using fallback")
            ai_text = sarvam_ai_service._generate_fallback_response(lead_speech, call_data["lead_info"].dict())

        call_data["conversation_history"].append({"role": "ai", "content": ai_text})

        # Try Sarvam AI TTS, fall back to Twilio TTS
        try:
            audio_bytes = await sarvam_ai_service.text_to_speech(text=ai_text, language="en-IN")
            ...
            with open(audio_path, "wb") as f:
                f.write(audio_bytes)
            audio_url = f"{webhook_base}/static/audio/{audio_filename}"
            print(f"✅ Sarvam AI TTS audio: {audio_url}")
            twiml = f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Gather input="speech" action="{webhook_base}/api/twilio/process-speech" method="POST" timeout="5" speechTimeout="auto" language="en-IN">
    <Play>{audio_url}</Play>
  ...
  <Redirect method="POST">{webhook_base}/api/twilio/voice</Redirect>
            ...
            print(f"⚠️ Sarvam AI TTS failed: {tts_err}, using Twilio TTS")
            twiml = f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Gather input="speech" action="{webhook_base}/api/twilio/process-speech" method="POST" timeout="5" speechTimeout="auto" language="en-IN">
    <Say voice="Polly.Aditi-Neural" language="en-IN">{ai_text}</Say>
  </Gather>
  <Redirect method="POST">{webhook_base}/api/twilio/voice</Redirect>
</Response>"""
        ...
        import traceback
        print(f"❌ PROCESS-SPEECH ERROR: {e}")
        traceback.print_exc()
        webhook_base = os.getenv("WEBHOOK_BASE_URL", "https://sarvam-ai-backend-nj0q.onrender.com")
        return Response(
            content=f'<?xml version="1.0" encoding="UTF-8"?><Response><Gather input="speech" action="{webhook_base}/api/twilio/process-speech" method="POST" timeout="5" language="en-IN"><Say voice="Polly.Aditi-Neural" language="en-IN">Could you please repeat that?</Say></Gather></Response>',
            media_type="application/xml"
        )
